...
export function ModalDigitalKassa({ open, onOpenChange }: IModalProfileProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="p-4 flex flex-col gap-6">
          <ModalAlert>
            <span>
              Регистрируйтесь по нашей партнерской ссылке.
              <br />
              Чем больше мерчантов будет подключено через систему Unitpay, тем
              ниже будет ежемесячная стоимость оплаты онлайн-кассы.
            </span>
          </ModalAlert>
          <ModalInstruction />
          ...
          <Input label="ID группы" className="w-full max-w-full" />
          <div className="flex gap-6 flex-wrap">
            <Input
              label="Актор в Digitalkassa"
              className="grow-1 basis-[200px] w-full"
            />
            <Input label="Токен" className="grow-1 basis-[200px] w-full" />
          </div>
